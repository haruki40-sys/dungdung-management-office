...
let dayIndex=0,sceneIndex=0,postIndex=0,resultIntroIndex=0,epiIndex=0,view='cover',selected=0,answers={},assignments={},log=[];
const STORAGE_KEY='office_v10_apply',LEGACY_BACKUP_KEY='office_v10_apply_legacy_v1_2_1',AUDIO_KEY='office_audio_enabled';
...
function progressSnapshot(){return{schema:2,version:'v1.3.2',dayIndex,sceneIndex,postIndex,resultIntroIndex,epiIndex,view,selected,answers,assignments,log}}
...
function showDayBreak(){hide();view='day-break';const day=days[dayIndex];chapter.textContent=`${day.day} · 시작`;stage.className=`stage ${day.place}`;$('day-break-kicker').textContent=`DAY ${dayIndex+1}`;$('day-break-day').textContent=day.day;$('day-break-title').textContent=day.title;dayBreak.classList.remove('hidden');doyun.classList.add('hidden');npc.classList.add('hidden');setProgress(5+Math.round(dayIndex/days.length*76));prev.disabled=true;historyOpen.disabled=dayIndex===0&&!log.length}
...
function renderResultIntro(){view='result-intro';renderDialogue(resultIntro[resultIntroIndex],'1막 · 관리계획 실행',90+Math.round((resultIntroIndex+1)/resultIntro.length*2));prev.disabled=false}
...
function currentScene(){if(view==='main')return resolve(days[dayIndex].scenes[sceneIndex]);if(view==='post')return resolve(days[dayIndex].choices[selected].post[postIndex]);if(view==='result-intro')return resolve(resultIntro[resultIntroIndex]);if(view==='epilogue')return resolve(epilogue[epiIndex])}
...
function advance(){logScene(currentScene());if(view==='main'){const day=days[dayIndex];if(sceneIndex<day.scenes.length-1){sceneIndex++;if(dayIndex===0&&sceneIndex===firstDayScene)showDayBreak();else renderMain()}else if(day.choices)showChoices();else showReflection(day.result,day.thought)}else if(view==='post'){const post=days[dayIndex].choices[selected].post;if(postIndex<post.length-1){postIndex++;renderPost()}else{const choice=days[dayIndex].choices[selected];showReflection(choice.result,choice.thought)}}else if(view==='result-intro'){if(resultIntroIndex<resultIntro.length-1){resultIntroIndex++;renderResultIntro()}else showPlanResult()}else if(view==='epilogue'){if(epiIndex<epilogue.length-1){epiIndex++;renderEpilogue()}else showEnding()}}
...
function confirmPlan(){if(!Object.keys(assignments).length){$('plan-warning').textContent='적어도 한 곳은 골라보세요.';return}resultIntroIndex=0;renderResultIntro()}
